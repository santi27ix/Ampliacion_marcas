...
var dado1 = Math.floor(Math.random() * 12) + 1;
var dado2 = Math.floor(Math.random() * 12) + 1;
...
if (suma >= 15) {
  ...
  var respuesta = prompt('Adivine su número aleatorio entre 1 y 5');
  var intentos = 0;
  intentos + 1;
  ...
  if (respuesta == num_aleatorio) {
    ...
    var num3bits = Math.floor(Math.random() * 3).toString(2).padStart(3, "0");
    var respuesta2 = prompt('Cuál es la clave de tres bits?');
  } else {
  ...
  var respuesta3 = prompt('¿Cuantas personas habitan en EE.UU? (en millones)');
  var pregunta = 300;
